...
- **확인 카드**(v2.43, 2026-09-11): 노란불(확인 필요)이면 데몬 `daemon/approval.mjs` `parseApproval()`이 화면 아래 글자에서 질문·선택지·누를 키를 뽑아 `rec.prompt`·`{type:'prompt'}` 방송으로 주고, `app/approval.js`가 대화 아래 `#approval` 카드(질문 원문 + 키 버튼 + 터미널에서 보기)를 그린다. 버튼·단축키(입력창 비었을 때 1~9·y·n·a)·Esc는 모두 터미널 보기와 같은 웹소켓 `input`으로 키 한 글자를 보낸다 — **승인 전용 API를 따로 만들지 않는다.** 모르는 형식은 raw(원문+Enter/Esc)로 떨어지며, 새 대화상자 형식을 만나면 `scripts/check-approval.mjs`에 샘플을 더해 파서를 넓힌다(`npm run verify:approval`). 상세 = `docs\구현계획.md` v2.43.
...
- **중앙 애니메이션 자동 조절**(v2.48, 2026-09-12 발열 사건): 비용은 무대 종류가 아니라 **별 수(밀도)** 다(headless 실측: 160%면 어느 무대든 2배). `app/stars.js`가 프레임 시간을 재서 **컴퓨터별 상한 밀도**(`localStorage['iris.stage.cap']`)를 정하고 실제 별 수 = min(사용자 밀도, 상한); 단계 = 별 0.1씩↓ → 50%에서 30fps → 30%까지(올릴 땐 역순, 20초 여유마다). 프레임 상한 = 포커스 잃음 10 · 배터리 30 · 평소 60. 별 배열은 사용자 밀도로 만들어 섞고 앞 live개만 그린다(상한 변경 때 자리 재추첨 없음). `pauseWhenDim` 기본 켬. 새 무대를 추가할 때 스프라이트(drawImage) 최적화는 쓰지 않는다(GPU 없는 환경에서 5배 느림). 검사 = `npm run verify:stage`(playwright 있을 때 headless CPU 4배 느림 시험 포함). 상세 = `docs\구현계획.md` v2.48.
